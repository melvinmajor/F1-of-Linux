...
void init_step(SharedStruct *shared_struct, int step) {
    ...
    if (step == P1) strcpy(step_name, "P1");
    else if (step == P2) strcpy(step_name, "P2");
    else if (step == P3) strcpy(step_name, "P3");
    else if (step == Q1) strcpy(step_name, "Q1");
    else if (step == Q2) strcpy(step_name, "Q2");
    else if (step == Q3) strcpy(step_name, "Q3");
    else if (step == RACE) strcpy(step_name, "race");

    if (step == Q2) number_of_cars_allowed = 15;
    else if (step == Q3 || step == RACE) number_of_cars_allowed = 10;
    else number_of_cars_allowed = 20;
...
int cars_running(Car *cars) {
    ...
    for (int i = 0; i < NUMBER_OF_CARS; ++i) {
        Car *car = &cars[i];
        if (!car->race_steps[current_step].allowed) continue;
        if (!car->race_steps[current_step].done) ++count;
    }
    return count;
}
